...
@app.route('/buscar-escolas-cep', methods=['POST'])
def buscar_escolas_cep():
    """Find schools near a given CEP using AwesomeApi and Excel data"""
    import pandas as pd
    import requests
    import math
    ...
    try:
        # Buscar coordenadas do CEP usando AwesomeApi
        cep_api_url = f"https://cep.awesomeapi.com.br/json/{cep}?token=67a8853ca627a5164edec7831f2bf7f2114edfced186eb70eb00db219a05b342"
        response = requests.get(cep_api_url, timeout=10)
        
        if response.status_code != 200:
            return jsonify({'error': 'Erro ao consultar CEP'}), 400
        
        cep_data = response.json()
        user_lat = float(cep_data['lat'])
        user_lng = float(cep_data['lng'])
        user_city = cep_data['city']
        user_state = cep_data['state']
        
        # Carregar dados das escolas do Excel
        try:
            df_escolas = pd.read_excel('attached_assets/escolas.xlsx')
            ...
            # Calcular distâncias e filtrar escolas próximas
            escolas_proximas = []
            
            for _, escola in df_escolas.iterrows():
                try:
                    escola_lat = float(escola['Latitude'])
                    escola_lng = float(escola['Longitude'])
                    
                    distancia = calcular_distancia(user_lat, user_lng, escola_lat, escola_lng)
                    
                    if distancia <= 50:  # Escolas num raio de 50km
                        escolas_proximas.append({
                            'nome': escola['Escola'],
                            'codigo_inep': escola['Codigo INEP'],
                            'uf': escola['UF'],
            ...
            escolas_proximas.sort(key=lambda x: x['distancia'])
            escolas_proximas = escolas_proximas[:5]
            ...
            app.logger.error(f"Erro ao processar arquivo de escolas: {e}")
            return jsonify({'error': 'Erro ao processar dados das escolas'}), 500
            
    except requests.exceptions.RequestException as e:
        app.logger.error(f"Erro na API de CEP: {e}")
